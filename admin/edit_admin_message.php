...
else 
{
	...
	$query="SELECT * FROM admin_get_message WHERE tid=$tid";
...
<form action="doedit_admin_message.php?tid=<?php echo $tid?>" method=post name="f" onsubmit="return check()" enctype="multipart/form-data">
<tr class=b>
...
$sql="select * from admin_get_message_images where mid=$tid and filetype='0'";
	...
	else
	{
		while($obj=mysql_fetch_array($r))
		{
			...
			echo "<input type='checkbox' name='checkimg[]' value='".$obj["tid"]."'><a href=message_image/big/".$obj["filename"]." target=_blank><img src=message_image/small/".$obj["filename"]."></a>&nbsp;&nbsp;";
			}
		}
	}
?>
