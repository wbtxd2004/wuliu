...
{
	$temp=$checkimg[$i];
	$query="SELECT filename FROM member_get_message_images WHERE tid=$temp";
	$result=mysql_db_query($DataBase,$query);
	$row=mysql_fetch_array($result);
	unlink("message_image/big/".$row["filename"]);
	unlink("message_image/small/".$row["filename"]);
	$query="DELETE FROM member_get_message_images WHERE `tid`=$temp";
...
$query="SELECT filename FROM member_get_message_images WHERE mid=$tid";
	$result=mysql_db_query($DataBase,$query);
...
{
	$temp2=$checkaudio[$i];
	$query2="SELECT filename FROM member_get_message_images WHERE tid=$temp2";
	$result2=mysql_db_query($DataBase,$query2);
	$row2=mysql_fetch_array($result2);
	unlink("message_audio/".$row2["filename"]);
	//unlink("message_image/small/".$row2["filename"]);
	$query2="DELETE FROM member_get_message_images WHERE `tid`=$temp2";
...
$query="SELECT * FROM member_get_message_images WHERE mid='$tid'";
...
$query="UPDATE member_get_message SET `biaoti`='$biaoti',`info`='$info' WHERE `tid`='$tid'";
$result = mysql_db_query($DataBase, $query);
if($result){
echo"私信编辑成功！";
