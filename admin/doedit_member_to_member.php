...
{
	$temp=$checkimg[$i];
	$query="SELECT filename FROM member_to_member_images WHERE tid=$temp";
	$result=mysql_db_query($DataBase,$query);
	$row=mysql_fetch_array($result);
	unlink("message_image/big/".$row["filename"]);
	unlink("message_image/small/".$row["filename"]);
	$query="DELETE FROM member_to_member_images WHERE `tid`=$temp";
...
$query="SELECT filename FROM member_to_member_images WHERE mid=$tid";
	$result=mysql_db_query($DataBase,$query);
...
for($i=0;$i<$up_num;$i++)
{
	...
	if($type=="jpg" or $type=="jpeg")	$src_image=imagecreatefromjpeg($dirname);
      ...
      $dst_image=ImageCreateTrueColor($dstw,$dsth);
	   ...
	   $query="INSERT INTO member_to_member_images(`mid`,`filename`) VALUES('$tid','$insert_name') ";
		mysql_db_query($DataBase, $query);
}
...
{
	$temp2=$checkaudio[$i];
	$query2="SELECT filename FROM member_to_member_images WHERE tid=$temp2";
	$result2=mysql_db_query($DataBase,$query2);
	$row2=mysql_fetch_array($result2);
	unlink("message_audio/".$row2["filename"]);
	//unlink("message_image/small/".$row2["filename"]);
	$query2="DELETE FROM member_to_member_images WHERE `tid`=$temp2";
...
$query="SELECT * FROM member_to_member_images WHERE mid='$tid'";
...
for($i=0;$i<$up_num2;$i++)
{
	...
	move_uploaded_file($tmp_name,$dirname);
	   ...
	   $query="INSERT INTO member_to_member_images(`mid`,`filename`,`filetype`) VALUES('$tid','$insert_name','1') ";
	   echo $query ;
		mysql_db_query($DataBase, $query);
}
